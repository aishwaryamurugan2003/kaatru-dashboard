...
const GroupDashboardPage: React.FC = () => {
  ...
  const [groupName, setGroupName] = useState<string>(location.state?.groupName || groupId || "Group");
  const [loading, setLoading] = useState(!location.state?.groupName);

  useEffect(() => {
    // If we didn't receive groupName from navigation state, fetch it
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{groupName} Dashboard</h1>
      </div>
